...
/*   Updated: 2020/07/20 13:38:02 by minckim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	copy_entities(char **map, t_gamedata *g)
{
	...
	while (++i < g->size_x)
	{
		...
		while (++j < g->size_y)
		{
			...
			if (map[i][j] == '0' || ft_strchr("NEWS234", map[i][j]))
...
void	init_entity(t_gamedata *g, char **map_arr)
{
	...
	g->sprite_ap = create_sprite(g->texture, TYPE_SPRITE_AP);
	g->sprite_42 = create_sprite(g->texture, TYPE_SPRITE_42);
	g->sprite_an = create_sprite(g->texture, TYPE_SPRITE_AN);
	g->non = create_non();
	g->wall.origin = vec_new(0, 0, 0);
	g->floor.origin = vec_new(0, 0, 0);
	g->sprite_ap.origin = vec_new(0, 0, 0);
	g->sprite_42.origin = vec_new(0, 0, 0);
	g->sprite_an.origin = vec_new(0, 0, 0);
